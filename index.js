...
form.addEventListener("submit", function (event) {
  ...
  posterOutput.innerHTML = `Generating poster for ${movieTitle.value.trim()} in the style of ${
    artStyles.value
  }`;
  const prompt = `An imaginative poster inspired by the movie "${movieTitle.value.trim()}" rendered in the ${
    artStyles.value
  } art style`;
...
async function generatePoster(prompt) {
  try {
    ...
    posterOutput.innerHTML =
      "Sorry, an error occured while generating the poster.";
  }
}
